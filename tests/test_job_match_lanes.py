...
@pytest.mark.parametrize(
    ("hard_requirement_status", "match_confidence", "expected_lane"),
    [
        ("unmet", 100.0, "unmet"),
        ("pass", 60.0, "recommended"),
        ("not_applicable", 60.0, "recommended"),
        ("pass", 59.99, "pending"),
...
def test_job_match_api_exposes_score_confidence_and_pending_lane(
    ai_client,
    monkeypatch,
) -> None:
    ...
    # Audit score retains the prior all-requirements denominator.  The score
    # users rank by is normalized only over conditions with actual evidence.
    assert payload["total_score"] == 50.0
    assert payload["match_score"] == 76.92
...
def test_job_version_match_list_orders_the_three_lanes_and_their_rankings(
    ...
    status_sets = iter(
        [
            ...
            ["met", "met", "met"],
            # Recommended: preferred information is absent; same match score,
            # lower confidence, so it follows the first recommended record.
            ["met", "met", "unknown"],
            # Pending: an unknown must-have remains explicitly reviewable.
            ["unknown", "met", "met"],
    ...
    assert [item["match_score"] for item in payload] == [100.0, 100.0, 100.0, 65.0]
    assert [item["match_confidence"] for item in payload] == [100.0, 70.0, 65.0, 100.0]
