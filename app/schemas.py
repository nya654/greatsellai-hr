...
# A recruiter-selected demographic. Only ``male``/``female`` are surfaced as
# screening options; ``unknown`` is reserved for storage/schema completeness.
...
SourceTagRuleMatchKind = Literal[
    "sender_domain",
    "sender_address",
    "subject_keyword",
]
...
class SourceTagReference(ApiModel):
    """A safe, snapshot-backed channel label for recruiter-facing views."""

    source_tag_id: str
    display_name: str


class SourceTagCreate(ApiModel):
    ...
    @field_validator("display_name")
    @classmethod
    def valid_source_tag_name(cls, value: str) -> str:
        normalized = " ".join(value.split()).strip()
        if not normalized:
            raise ValueError("source tag name must not be blank")
        return normalized


class SourceTagPatch(ApiModel):
    ...
    @field_validator("display_name")
    @classmethod
    def valid_optional_source_tag_name(cls, value: str | None) -> str | None:
        if value is None:
            return None
        normalized = " ".join(value.split()).strip()
        if not normalized:
...
class SourceTagResponse(SourceTagReference):
    enabled: bool
...
class MailboxSourceTagRuleCreate(ApiModel):
    ...
    @classmethod
    def valid_source_tag_rule_value(cls, value: str) -> str:
        normalized = " ".join(value.split()).strip()
        if not normalized:
            raise ValueError("source tag rule value must not be blank")
        return normalized


class MailboxSourceTagRulePatch(ApiModel):
    source_tag_id: str | None = Field(default=None, min_length=1, max_length=64)
    match_kind: SourceTagRuleMatchKind | None = None
    ...
    @field_validator("match_value")
    @classmethod
    def valid_optional_source_tag_rule_value(cls, value: str | None) -> str | None:
        if value is None:
            return None
        normalized = " ".join(value.split()).strip()
        if not normalized:
...
class MailboxSourceTagRuleResponse(ApiModel):
    rule_id: str
...
class MailboxImportResponse(ApiModel):
    ...
    created_at: datetime
    source_tags: list[SourceTagReference] = Field(default_factory=list)
...
class ResumeFactsSubmission(ApiModel):
    ...
    # Demographics are collected so a recruiter can screen by gender and age.
    # They come from explicit resume lines only and stay behind the same
    # evidence-grounding checks as every other persisted fact.
    gender_raw: str | None = Field(default=None, max_length=16)
    gender_evidence_block_ids: list[str] = Field(default_factory=list, max_length=2)
    birth_date_raw: str | None = Field(default=None, max_length=32)
    birth_date_evidence_block_ids: list[str] = Field(default_factory=list, max_length=2)
    ...
    @field_validator("gender_raw", "birth_date_raw")
    @classmethod
    def valid_demographic_raw(cls, value: str | None) -> str | None:
        if value is None:
            return None
        cleaned = value.strip()
    ...
    def has_at_least_one_fact(self) -> "ResumeFactsSubmission":
        ...
        if self.gender_raw and not self.gender_evidence_block_ids:
            raise ValueError("gender_requires_evidence_block_id")
        if not self.gender_raw and self.gender_evidence_block_ids:
            raise ValueError("gender_evidence_requires_gender")
        if self.birth_date_raw and not self.birth_date_evidence_block_ids:
            raise ValueError("birth_date_requires_evidence_block_id")
        if not self.birth_date_raw and self.birth_date_evidence_block_ids:
            raise ValueError("birth_date_evidence_requires_birth_date")
        if not (
            self.education
            or self.experiences
            or self.skills
...
class CandidateSearchRequest(ApiModel):
    ...
    # A submission source is independent of factual screening and uses fixed
    # OR semantics: selecting BOSS and LinkedIn means "from either channel".
    # It never participates in the page-level all/any switch.
    source_tag_ids_any_of: list[str] = Field(default_factory=list, max_length=20)
    # Demographic screening. ``gender_in`` selects any listed gender (OR),
    # mirroring the institution multi-select. ``age_min``/``age_max`` bound the
    # candidate's age computed from an extracted birth date as of the search
    # day; both are optional and inclusive.
    gender_in: list[Gender] = Field(default_factory=list, max_length=3)
    age_min: int | None = Field(default=None, ge=16, le=80)
    age_max: int | None = Field(default=None, ge=16, le=80)
    ...
    def valid_v2_semantics(self) -> "CandidateSearchRequest":
        ...
        if (
            ...
            raise ValueError("age_max must not be earlier than age_min")
        if any(
...
class CandidateSearchItem(ApiModel):
    ...
    # Derived from the current resume's immutable mail-import facts.  This is
    # deliberately not a mutable Candidate attribute: one person can arrive
    # via several platforms or have several source-labelled resume versions.
    source_tags: list[SourceTagReference] = Field(default_factory=list)
