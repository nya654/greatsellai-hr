...
import { modelDisplayName, providerDisplayName } from "../ai-display";
...
const routeFeatures = [
  ...
  { value: "resume_extract_core", label: "简历核心信息提取", detail: "提取筛选所需的核心字段" },
...
function routeModelLabel(modelSlug: string, modelNames?: ReadonlyMap<string, string>) {
  if (!modelNames) return modelSlug;
  return modelNames.get(modelSlug) ?? "已归档模型";
}

function routeModelChain(targets: RouteTargetSummary[], modelNames?: ReadonlyMap<string, string>) {
  return targets.length ? targets.map((target) => routeModelLabel(target.model_slug, modelNames)).join(" → ") : "未配置";
}

function routeRetrySummary(targets: RouteTargetSummary[], modelNames?: ReadonlyMap<string, string>) {
  return targets.length
    ? targets.map((target) => `${routeModelLabel(target.model_slug, modelNames)}：${target.max_attempts} 次`).join("；")
    : "未配置";
}

function routeFallbackSummary(targets: RouteTargetSummary[], modelNames?: ReadonlyMap<string, string>) {
  const summaries = targets.flatMap((target) => {
    ...
    return [`${routeModelLabel(target.model_slug, modelNames)}：${labels.join("、")}`];
  });
...
export function AdminAiConfigurationPanel({
  ...
  const submitProvider = async (event: FormEvent) => {
    ...
    try {
      ...
      await finishSubmission(`模型服务「${providerDisplayName(created)}」已接入。`);
  ...
  return (
    <div className="admin-ai-config-layout">
      <aside className="admin-panel admin-ai-config-guide" aria-label="配置发布说明">
        ...
        <dl>
          <div><dt>模型服务</dt><dd>{providers.length} 个</dd></div>
          <div><dt>可用模型服务</dt><dd>{routableProviderCount} / {providers.length}</dd></div>
      ...
      <section className="admin-panel admin-ai-config-panel" aria-labelledby="admin-ai-config-title">
        ...
        <div className="admin-segmented admin-ai-config-tabs" aria-label="AI 配置操作">
          {([['provider', '1 模型服务'], ['model', '2 模型'], ['route', '3 发布规则']] as Array<[ConfigurationSection, string]>).map(([value, label]) => (
        ...
        {section === "provider" && <form className="admin-management-form admin-ai-config-form" onSubmit={submitProvider}>
          <div className="admin-ai-form-heading"><div><h3>接入模型服务</h3><p>目前支持标准兼容接口。凭据由服务器托管，此处仅登记引用名。</p></div><AdminStatus status="enabled" label="可创建" /></div>
          <div className="admin-form-grid">
            ...
            <label className="admin-ai-field-wide"><span>服务接口地址</span><input autoCapitalize="none" className="field" maxLength={1000} onChange={(event) => setProviderDraft({ ...providerDraft, endpoint_url: event.target.value })} placeholder="https://api.example.com/v1/chat/completions" required type="url" value={providerDraft.endpoint_url} /><small>仅支持公网 HTTPS 地址，不能在地址中填写认证信息。</small></label>
          ...
          {!!providers.length && <section className="admin-ai-provider-runtime" aria-label="模型服务连接状态">
            <div>
              ...
              <p>这里只显示连接是否就绪，不显示密钥。</p>
            </div>
            <ul className="admin-simple-list">
              {providers.map((provider) => (
                <li key={provider.provider_id}>
                  <span><strong>{providerDisplayName(provider)}</strong><small>{provider.credential_configured ? "服务端密钥已配置" : "等待服务端密钥配置"}</small></span>
                  <AdminStatus
                    status={provider.credential_configured ? "verified" : "warning"}
                    label={provider.credential_configured ? "已连接" : "待配置"}
                  />
                </li>
              ))}
            </ul>
          ...
          <div className="admin-form-actions"><button className="button button-primary" disabled={saving === "provider"} type="submit">{saving === "provider" ? <><i className="spinner" />正在创建</> : <><Icon name="plus" size={16} />接入模型服务</>}</button></div>
        </form>}
        ...
        {section === "model" && <form className="admin-management-form admin-ai-config-form" onSubmit={submitModel}>
          ...
          <div className="admin-form-grid">
            ...
            <label><span>服务商模型 ID</span><input autoCapitalize="none" className="field" maxLength={255} onChange={(event) => setModelDraft({ ...modelDraft, provider_model_id: event.target.value })} placeholder="例如：deepseek-chat" required value={modelDraft.provider_model_id} /></label>
        ...
        {section === "route" && <form aria-busy={routeVersionsState === "loading" || saving === "route"} className="admin-management-form admin-ai-config-form" onSubmit={prepareRouteReview}>
            ...
            <div className="admin-ai-route-review-table-wrap">
              <table className="admin-ai-route-review-table">
                ...
                <tbody>
                  ...
                  <tr><th scope="row">模型链</th><td>{routeReview.currentVersion ? routeModelChain(routeReview.currentVersion.targets, modelNames) : "未配置"}</td><td><strong>{routeModelChain(routeReview.payload.targets, modelNames)}</strong></td></tr>
                  <tr><th scope="row">最大尝试</th><td>{routeReview.currentVersion ? routeRetrySummary(routeReview.currentVersion.targets, modelNames) : "未配置"}</td><td>{routeRetrySummary(routeReview.payload.targets, modelNames)}</td></tr>
                  <tr><th scope="row">回退条件</th><td>{routeReview.currentVersion ? routeFallbackSummary(routeReview.currentVersion.targets, modelNames) : "未配置"}</td><td>{routeFallbackSummary(routeReview.payload.targets, modelNames)}</td></tr>
          ...
          <section className="admin-ai-route-history" aria-live="polite">
            ...
            {routeVersionsState === "ready" && !!routeVersions.length && <ol>{routeVersions.map((version) => <li key={version.route_policy_version_id}><span><strong>v{version.version}</strong><small>{routeModelChain(version.targets, modelNames)}</small></span><time dateTime={version.published_at}>{formatDate(version.published_at, true)}</time></li>)}</ol>}
          </section>
        </form>}
      </section>
    </div>
