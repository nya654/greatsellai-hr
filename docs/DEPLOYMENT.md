...
早期单账号版本可能已有候选人与原文件，但还没有任何已验证的正式平台管理员。身份迁移在这种状态下会安全停止，避免废弃共享账号后让历史数据失去可信访问者。

在部署前，由负责人使用**已经登录并完成邮箱验证的正式账号**的 `user_id`，在服务器本地、被 Git 忽略的环境文件中临时设置：

```text
RESUME_V3_LEGACY_WORKSPACE_ADOPTION_USER_ID=<该账号的 UUID>
```

该值不是邮箱、密码或令牌；它只会传给 `migrate` 容器。迁移会再次验证该账号不是 legacy 系统账号、仍处于启用状态且已验证邮箱，然后才会在同一事务内授予它平台管理员和历史工作区管理员权限，撤销共享身份并写入不含候选人内容的审计事件。不存在、停用、未验证或未显式配置的账号都会使迁移在写入前失败。

迁移成功后立即从生产与 Staging 的环境文件删除该变量。不能根据邮箱、注册时间或普通用户列表自动选择接管人，也不能把共享账号标记为已验证来绕过这一步。
