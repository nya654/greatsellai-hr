...
export function AdminPlansPage() {
  ...
  return (
    <section className="admin-page-frame" aria-labelledby="admin-plans-title">
      <AdminPageHeader actions={<button className="button" onClick={() => void load()} type="button"><Icon name="refresh" size={16} />刷新套餐</button>} description="管理价格、默认试用天数、注册开放选项和套餐功能范围。" title="套餐与试用" />
      ...
      {state === "ready" && !!plans.length && (
        <div className="admin-plan-layout">
          ...
          {draft && <form className="admin-panel admin-plan-editor" onSubmit={(event) => void save(event)}>
            <div className="admin-section-heading"><div><h2>编辑 {draft.name}</h2><p>保存后立即生效为服务端的正式配置。</p></div><span>{draft.code}</span></div>
    ...
    <section className="admin-page-frame admin-page-frame-wide" aria-labelledby="admin-ai-title">
      ...
      {state === "ready" && tab === "runs" && (
        <div className="admin-table-panel">
          <div className="admin-table-note"><span>最近 {runs.length} 条运行</span><small>不含提示词、简历或模型输出；如需查看各模型的 Token，请在「Token 用量」中按模型服务和模型筛选。</small></div>
      ...
      {state === "ready" && tab === "resources" && <div className="admin-resource-stack">
        ...
        <section className="admin-table-panel">
          <div className="admin-table-note"><span>功能调用路由</span><small>{routes.length} 项功能</small></div>
          {routes.length ? <ul className="admin-resource-list">{routes.map((item) => <li key={item.policy_id}><span><strong>{routeDisplayName(item)}</strong><small>{routeDisplayDescription(item)} · 当前版本 {item.current_version ?? "—"}</small></span><AdminStatus status={item.is_enabled ? "enabled" : "disabled"} label={item.is_enabled ? "已启用" : "已停用"} /></li>)}</ul> : <DataTableEmpty description="尚未发布功能调用路由。" />}
        </section>
      </div>}
...
export function AdminAuditPage() {
  ...
  return (
    <section className="admin-page-frame admin-page-frame-wide" aria-labelledby="admin-audit-title">
      ...
      {state === "ready" && !!items.length && <div className="admin-table-panel"><div className="admin-data-table-scroll"><table className="admin-data-table admin-audit-table"><thead><tr><th>时间</th><th>操作人</th><th>操作</th><th>目标</th><th>公司</th><th>原因</th><th>变更</th></tr></thead><tbody>{items.map((item) => <tr key={item.audit_id}><td>{formatDate(item.created_at,true)}</td><td title={item.actor_user_id || undefined}>{item.actor_kind === "system_migration" ? "系统迁移" : shortId(item.actor_user_id)}</td><td>{auditActionName(item.action)}</td><td><strong>{item.target_type}</strong><small title={item.target_id || undefined}>{shortId(item.target_id)}</small></td><td>{companyName(item.organization_id)}</td><td className="admin-audit-reason">{item.reason || "未填写"}</td><td><details className="admin-audit-change"><summary>查看变更</summary><div><section><h3>修改前</h3><pre>{statePreview(item.before_state)}</pre></section><section><h3>修改后</h3><pre>{statePreview(item.after_state)}</pre></section><p>请求编号：{item.request_id || "—"}</p></div></details></td></tr>)}</tbody></table></div><AdminPagination limit={PAGE_SIZE} offset={offset} onChange={setOffset} total={total} /></div>}
    </section>
  );
}
