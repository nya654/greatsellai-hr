...
SKIP_NEVER_SCORED = "never_scored"
...
def retry_resume_failed(
    ...
    *,
    ...
    """Dispatch retries for every currently failed pipeline of one resume.
    ...
    an in-progress replacement blocks AI extraction, and summary/score retries
    only run for a resume that is currently active and extraction-ready.
    """
    ...
    # 3. Failed/unavailable automatic summary → re-queue for current facts.
    ...
    if (
        ...
        and summary_status in {SUMMARY_JOB_FAILED, SUMMARY_JOB_UNAVAILABLE}
    ):
        request_resume_summary_job(session, resume=resume, settings=settings)
        actions.append(ACTION_SUMMARY)

    # 4. Failed template scoring → re-queue with the same template.
