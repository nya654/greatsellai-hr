#!/usr/bin/env bash
# Archive one narrowly-defined structured pending marker only after proving its
# requested target is already the authoritative, healthy production runtime.
# This is neither a deploy nor a rollback and never changes application,
# database, upload, Docker-network, or container state.
set -Eeuo pipefail
...
usage() {
  cat <<'EOF'
Usage: scripts/finalize-healthy-pending-release.sh <pending-prod-tag> <pending-commit> [options]
...
This command is restricted to a structured pending release whose exact target
already matches the complete current release and is still healthy. It verifies
the staged source, paired backup, target images, service states, proxy topology,
Caddy configuration, HTTPS health, unauthenticated session protection, and the
protected original-file endpoint before atomically archiving only the pending
marker. It does not build, migrate, stop, start, recreate, restore, remove, or
reconfigure Docker resources, and it does not modify candidate data.
EOF
}
...
[[ "$confirmed" -eq 1 ]] || die "Pass --confirm to acknowledge healthy pending finalization."
...
remote_helper="$(ssh_run 'umask 077 && mktemp /tmp/greatsell-healthy-pending-finalize.XXXXXXXX')"
[[ "$remote_helper" =~ ^/tmp/greatsell-healthy-pending-finalize\.[A-Za-z0-9]{8}$ ]] || \
...
ssh_run "bash $(shell_quote "$remote_helper") finalize-healthy-pending-target $(shell_quote "$project_dir") $(shell_quote "$history_dir") $(shell_quote "$pending_tag") $(shell_quote "$pending_commit") FINALIZE_HEALTHY_PENDING_RUNTIME $(shell_quote "$source_tree_sha256")"

echo "Healthy pending target finalized: $pending_tag ($pending_commit)"
