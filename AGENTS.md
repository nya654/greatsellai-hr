...
- 连续合并多个 PR 时，每合并一个 PR，必须先 fetch 最新 `origin/main`，再将下一个待合并
  PR 更新或 rebase 到该基线、重新跑完 PR CI 后才能 squash merge。不得把仍以旧 main 为
  基线的绿灯 PR 直接合并；发布溯源门会正确拒绝这种未经组合验证的提交。
...
- AI 输出只能来自已提取且带证据的简历事实或面试记录，保持建议性质；不得编造，也不得
  自动拒绝或录用候选人。
- 常规前端控件（按钮、输入、选择、进度、标签、提示、弹窗、表格等）必须优先使用
  `@douyinfe/semi-ui-19`；如项目已有对应的 Semi 封装，使用该封装。禁止用 `div`、ARIA
  role 和自定义 CSS 手搓替代已有的 Semi 控件。只有 Semi 及项目封装均无法满足需求时，才可
  自定义实现，并须在 PR 中说明原因、保持可访问性并补充验证。
