...
> 这套 TCR 配置曾是生产镜像交接用的，现已被"静默拉 + 人工发"的直传 + staging 中转模型取代，
> 本文仅保留作历史参考。当前 `main` CI 已停止向 TCR 推送镜像；staging 在美国发布 Runner 直接
> 构建并流式传输镜像，生产晋级由 staging 主机中转把已验证镜像预加载到生产机、负责人手动
> `PROMOTE` 才发布（详见 [预发布与生产晋级](STAGING_RELEASE.md)）。以下配置与流程保留，供
> 审计或将来回退到 TCR 方案时使用。历史上它把 API/Caddy 镜像推送到腾讯云 TCR，让预发布和生产
> 按不可变 `repo@sha256:<manifest>` 拉取同一份镜像，不再把数百 MB 的 Docker 镜像作为 GitHub
> Actions artifact 下载并经 SSH 转发。
...
## 发布时发生的事（历史流程，已被直传 + staging 中转取代）

当前 `main` CI 已停止自动推送 TCR 镜像，staging 采用 direct 构建 + 流式传输，生产晋级走
staging 主机中转预加载 + 人工 `PROMOTE`。若未来回退到 TCR 方案，需重新接入以下链路：
